...
import { recovery } from '../data/plans';

export default function RecoveryPage({ color }) {
  return (
    <div style={{ paddingTop:52 }}>
      <Nav />
      <Hero badge="Post-Race" title="RECOVERY PLAN" subtitle="Hour 0 through Day 3 · You burned ~3,500–4,000 kcal" img="https://images.unsplash.com/photo-1506126613408-eca07ce68773?w=1400&q=80" accentColor={color} />
      <div style={{ maxWidth:720, margin:'0 auto', padding:'2rem 1.25rem 5rem' }}>
        {recovery.map((phase,pi) => (
        ...
        <div style={{ padding:'1.5rem', background:'var(--surface2)', border:'1px solid var(--border)', borderLeft:`3px solid ${color}`, borderRadius:10 }}>
          <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:'1.3rem', color:'var(--text)', marginBottom:'0.5rem', letterSpacing:'0.06em' }}>You're going to want to sign up for the next one</div>
          <p style={{ color:'var(--muted)', fontSize:'0.92rem', lineHeight:1.65 }}>The legs still sore, the strange empty hunger, the replaying of the race — that's your body telling you it's ready to do it again. Rest it. Feed it. Sleep. Then start planning.</p>
          <div style={{ marginTop:'1rem', fontFamily:"'JetBrains Mono',monospace", fontSize:'0.72rem', color }}>Full recovery: 2–4 weeks · Return to easy running: ~10 days</div>
        </div>
      </div>
    </div>
  );
