...
export default function DinnerPage({ color }) {
  return (
    <div style={{ paddingTop:52 }}>
      <Nav />
      <Hero badge="Night Before the Race" title="PRE-RACE DINNER" subtitle={`${D.restaurant} · ${D.address}`} img="https://images.unsplash.com/photo-1555396273-367ea4eb4db5?w=1400&q=80" accentColor={color} />
      <div style={{ maxWidth:720, margin:'0 auto', padding:'2rem 1.25rem 5rem' }}>
        <div style={{ background:'var(--surface2)', border:`1px solid ${color}40`, borderLeft:`3px solid ${color}`, borderRadius:10, padding:'1rem 1.25rem', marginBottom:'2.5rem' }}>
          <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:'0.68rem', color, letterSpacing:'0.15em' }}>STRATEGY · </span>
        ...
        <div style={{ padding:'1rem 1.25rem', background:'var(--surface2)', border:'1px solid var(--border)', borderRadius:10, marginBottom:'0.5rem' }}>
          <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:'1.1rem', color:'var(--text)', marginBottom:'0.3rem' }}>{D.starter.name}</div>
          <p style={{ color:'var(--muted)', fontSize:'0.9rem', lineHeight:1.55 }}>{D.starter.desc}</p>
        </div>
        ...
        <div style={{ marginTop:'2rem', padding:'1rem 1.25rem', background:'var(--surface3)', border:'1px solid var(--border)', borderRadius:8 }}>
          <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:'0.66rem', color, letterSpacing:'0.15em' }}>PORTION NOTE · </span>
